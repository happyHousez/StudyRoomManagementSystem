...
//点击借阅图书时执行
function borrow() {
    var url =getProjectPath()+ "/room/borrowRoom";
    $.post(url, $("#borrowBook").serialize(), function (response) {
        alert(response.message)
        if (response.success == true) {
            window.location.href = getProjectPath()+"/room/searchRooms";
        }
    })
}
...
//查询id对应的自习室信息，并将自习室信息回显到编辑或借阅的窗口中
function findBookById(id,doname) {
    resetStyle()
    var url = getProjectPath()+"/room/findById?id=" + id;
    $.get(url, function (response) {
        ...
        if(doname=='edit'){
            ...
            $("#eblocation").val(response.data.location);
            $("#ebcapacity").val(response.data.capacity);
            $("#ebphone").val(response.data.phone);
            $("#ebopenTime").val(response.data.openTime);
            $("#ebcloseTime").val(response.data.closeTime);
            $("#ebdesc").val(response.data.description);
        ...
        if(doname=='borrow'){
            ...
            $("#blocation").val(response.data.location);
            $("#bcapacity").val(response.data.capacity);
            $("#bphone").val(response.data.phone);
            $("#bopenTime").val(response.data.openTime);
            // $("#bopenTime").val(response.data.openTime);
...
function addOrEdit() {
    ...
    if (ebid > 0) {
        var url = getProjectPath()+"/room/editRoom";
    ...
    //如果表单中没有图书id，说明本次为添加操作
        ...
        var url = getProjectPath()+"/room/addRoom";
        $.post(url, $("#addOrEditBook").serialize(), function (response) {
            alert(response.message)
            if (response.success == true) {
                window.location.href = getProjectPath()+"/room/searchRooms";
...
// 删除用户
function delectUser(uid)
{
    var r = confirm("确定删除用户?");
    if (r == true) {
        var url = getProjectPath()+"/user/deleteUser?id=" + uid;
        $.get(url, function (response) {
            alert(response.message)
            if (response.success == true) {
                window.location.href = getProjectPath()+"/user/search";
